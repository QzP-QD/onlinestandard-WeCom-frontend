<template>
  <div>
    ...
    <div v-for="item in items">
      <van-cell>
        <template slot="title">
          <span class="custom-title" @click="toDetail">{{ item.business }}-{{ item.name }}</span><br/>
        ...
        <div style="text-align: right;margin-right: 10px;">
          ...
          <br/>
          <van-button round type="info" size="mini" @click="selectAsCompare(item)":disabled="isdisabled">选择为对比项</van-button>
        </div>
      </van-cell>
    </div>
  </div>
